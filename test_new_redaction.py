    ...
    for i in range(batch_size):
        ...
        for i_temp in non_paded_attention:
            ...
            if curr_word not in ranked_masked_word_list:
                # words_used_crr_tmp.append(curr_word)
        ...
        # words_to_mask = []
        # for idx in ranked_masked_word_list:
        #     words_to_mask.append()
        # IPython.embed()
        updated_sent = ""
...
def get_old_redacted_data(data_dir,sentence_transformer,mask_percentage:int):
    ...
    p_feat, q_feat = normaliseData(p_feat_sampled, q_feat_sampled, norm='l1')
...
def bootstrap_data_get_idx(p_feat_data,q_feat_data,alphas):
    ...
    eps = get_line_plots_using_kneedle(data_c_sorted,0.0005)
    return eps,0.0005
...
if __name__ == '__main__':
    ...
    save_dir = parent_dir+"medal/sent_full_new" # dir to save
    ...
    for mask_perc in range(0,101,10):
        ...
        if mask_fraction > 0:
            p_text_sampled = compute_masked_sentences(p_val, masking_network, sentence_transformer,
                                                                        device,gt="p",mask_fraction=mask_fraction,vectorizer=vectorizer)
            q_text_sampled = compute_masked_sentences(q_val, masking_network, sentence_transformer,
                                                                    device,gt="q",mask_fraction=mask_fraction,vectorizer=vectorizer)
        else:
            p_text_sampled = p_val
            q_text_sampled = q_val
        q_text_sampled_array = np.asarray(q_text_sampled)
        p_text_sampled_array = np.asarray(p_text_sampled)
    ...
    # np.save("{}/{}.npy".format(diver_gence_dir,"political_data"), dataToPlot)
